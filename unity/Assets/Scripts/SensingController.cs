...
public class SensingController : MonoBehaviour
{
    ...
    void Start() {
        sp = new SerialPort("/dev/ttyUSB0", 115200);
        ...
        if (sp.IsOpen) {
            try {
                if (sp.BytesToRead > 0) {
                    ...
                    //print(bending);
                    float index_angle = map(bending, -10,-30, 20,-50);
                    indexJoint2.transform.DOLocalRotate( new Vector3(0, 0, index_angle), 0.9f);
...
//                    heading = (heading * 180.0f) / (float)Math.PI; // => [-180 ; +180]
                    float heading = float.Parse(data[4]);
                    //print(heading);
                    float hand_angle = map(heading, -30,30, 120,200);
                    hand.transform.DOLocalRotate( new Vector3(30, hand_angle, 0), 0.8f);
                }
